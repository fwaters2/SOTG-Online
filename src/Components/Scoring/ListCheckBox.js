...
export default function ListCheckBox({
  ...
  const isChecked = formResponses[examples]
    .map(x => x.category === category && x.categoryScore === categoryScore && x.index === index)
    .includes(true);
  ...
  const handleClick = () => {
    if (isChecked) {
      const filteredExamples = formResponses[examples].filter(
        x => !(x.category === category && x.categoryScore === categoryScore && x.index === index)
      );
      ...
      setFormResponses({
        ...formResponses,
        [examples]: [...formResponses[examples], { category, categoryScore, index }],
      });
    }
  };
  return (
