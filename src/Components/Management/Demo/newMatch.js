export default function newMatch(previousMatches, newScore) {
  ...
  function query(spiritScore) {
    const matchToUpdateStep1 = previousMatches
      ...
      .filter(
        team =>
          (team.team1 === spiritScore.myTeam || team.team1 === spiritScore.opponent) &&
          (team.team2 === spiritScore.myTeam || team.team2 === spiritScore.opponent)
      );
    const matchToUpdate = matchToUpdateStep1
      // grab match where the score is missing
      .filter(uncompleted => {
        // find out if they are team 1 or team 2
        const teamNo = uncompleted.team1 === spiritScore.myTeam ? 'team1' : 'team2';
    ...
    switch (status) {
      ...
      case 'updateMatch': {
        const team = matchToUpdate[0].team1 === spiritScore.myTeam ? 'team1' : 'team2';
        previousMatches.find(x => x.id === matchToUpdate[0].id)[`${team}Submitted`] = true;
        previousMatches.find(x => x.id === matchToUpdate[0].id).completed = true;
