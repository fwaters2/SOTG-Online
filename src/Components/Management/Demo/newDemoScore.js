...
export default function newDemoScore(initialEvents, langStrings, spiritScores, previousMatches) {
  const randomTeam =
    initialEvents[0].teams[Math.floor(Math.random() * initialEvents[0].teams.length)];
  ...
  function randomFeedback() {
    const categories = ['rules', 'fouls', 'attitude', 'fairness', 'communication'];
